...
Then(/^I validate page header \"([^\"]*)\"$/, async function (header:string) {
    await  expect( appData.header).toHaveText(header)
...
When(/^I select gender (.+) years  (.+) date (.+) favourite chai (.+) and reason (.+)$/, async function (gender:string, yrs:string,date:string,favchai:string, reason:string) {
    ...
    await appData.selectdropdown(await appData.gender_ratio,gender)
